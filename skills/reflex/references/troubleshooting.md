...
|---|---|---|
| `missing_doable_key` | No Bearer reached the server | The MCP client launched without the Doable key. Hardcode it in `.mcp.json`, or export `QA_DOABLE_API_KEY` and relaunch the client. See references/connect.md. |
...
exceed the hard time ceiling — see staleness in references/workflow.md.)
...
`.mcp.json` or export the env var) and reconnect. See references/connect.md.
